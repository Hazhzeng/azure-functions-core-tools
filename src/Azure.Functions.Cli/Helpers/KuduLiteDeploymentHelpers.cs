...
namespace Azure.Functions.Cli.Helpers
{
    ...
    {
        public static async Task<string> GenerateRestrictedToken(HttpClient client, Site functionApp)
        {
            using (var request = new HttpRequestMessage(HttpMethod.Get, new Uri("/", UriKind.Relative)))
            {
                var response = await client.SendAsync(request);
                string authKey = await response.Content.ReadAsStringAsync();
                return SimpleTokenHelper.CreateToken(DateTime.UtcNow.AddDays(1), authKey);
            }
        }
